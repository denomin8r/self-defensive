...
export const updateMovement = (delta, controls, camera, wallGroup, paintingGroup) => {
  ...
  if (checkCollision(camera, wallGroup)) {
    // reset the camera position to the previous position. The `previousPosition` variable is a clone of the camera position before the movement. We use `copy` instead of `set` because `set` will set the position to the same object, so if we change the previousPosition, the camera position will also change. `copy` creates a new object with the same values as the previousPosition.
    camera.position.copy(previousPosition); 
  }

  if (checkCollision(camera, paintingGroup)) {
    camera.position.copy(previousPosition);
  }
};
...
export const checkCollision = (camera, planeObjectGroup) => {
  ...
  for (const wall of planeObjectGroup.children) {
